...
router.get('/live', function (req, res, next) {  // Alle Parkhäuser
    let parkhausobj = {};
    connection.query("Select * from parkhaus_info where winkel > 1", function (error, results, fields) {
        parkhausobj.parkhaus = [];
        for (let i = 0; i <= results.length - 1; i++) {   // Loop durch die Info
            parkhausobj.parkhaus.push(results[i]);
            delete parkhausobj.parkhaus[parkhausobj.parkhaus.length - 1].id; // Live Daten für jedes Parkhaus holen.
            connection.query("select frei,gesamt,status from parkhaus_data where lfdnr='" + parkhausobj.parkhaus[parkhausobj.parkhaus.length - 1].parkhaus_id + "' order by id desc limit 1", function (error, result, fields) {
                parkhausobj.parkhaus[i].live = result[0]; // Anhängen an das Parkhausobj.
...
router.get('/', function (req, res, next) {  // Alle Parkhäuser
    ...
    connection.query("Select * from parkhaus_info where winkel > 1", function (error, results, fields) {
        ...
        if (error) {
            ...
            console.log(parkhausobj.parkhaus);
...
router.get('/id:id', function (req, res, next) {//Abruf Parkhaus ID
